...
export async function main(ns) {
    ...
    while (ramAvailable < ramNeeded) {
        ...
        grow.threads = Math.ceil(ns.growthAnalyze(h, 1 / (1 - take), cores))
    ...
    let printState = () => {
        ns.tprint(`m: ${ns.getServerMoneyAvailable(h)}  s: ${ns.getServerSecurityLevel(h)}`)
    }

    // schedule.push({ t: 0, r: weaken.exec },
    //     { t: weaken.time - grow.time - interval / 3, r: grow.exec },
    //     { t: weaken.time - hack.time - interval * 2 / 3, r: hack.exec },
    //     { t: interval, r: weaken.exec },
    //     { t: interval + weaken.time - grow.time - interval / 3, r: grow.exec },
    //     { t: interval + weaken.time - hack.time - interval * 2 / 3, r: hack.exec },
    //     { t: 2 * interval, r: weaken.exec },
    //     { t: 2 * interval + weaken.time - grow.time - interval / 3, r: grow.exec },
    //     { t: 2 * interval + weaken.time - hack.time - interval * 2 / 3, r: hack.exec }
    //     // { t: weaken.time - interval, r: printState },
    //     // { t: weaken.time - interval / 2, r: printState },
    //     // { t: weaken.time, r: printState },
    //     // { t: weaken.time + interval / 2, r: printState },
    // )

    for (let i = 0; i * interval < weaken.time; i += interval) {
        schedule.push(
            { r: weaken.exec, t: i },
            { r: grow.exec, t: i + weaken.time - grow.time - interval / 3 },
            { r: hack.exec, t: i + weaken.time - hack.time - interval * 2 / 3 },
        )
    }
