...
export async function main(ns) {
    ...
    renderTable(ns.tprintf, tasks.map((t) => {
        let r = {
            task: t.name,
            respect: 100 * Math.max(...members.map((m) => calculateRespectGain(gang, m, t))),
            money: Math.max(...members.map((m) => calculateMoneyGain(gang, m, t))),
            wanted: 100 * Math.max(...members.map((m) => calculateWantedLevelGain(gang, m, t))),
        }
        return r;
    }))
    ...
    ns.tprintf(members.toSorted((a, b) => calculateRespectGain(gang, b, t) - calculateRespectGain(gang, a, t))[0].name)
}